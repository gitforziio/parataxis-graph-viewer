...
const MyVis = class MyVis {
  ...
  static VisElement = class VisElement {
    ...
    docNode(...args) {
      if (!!args[0]) {
        ...
        this.__selection__ = MyVis.D3.select(this.__docNode__);
      };
  ...
  _makeChars() {
    ...
    if (text?.length) {
      chars.forEach((char, idx)=>{
        things.chars.push(new MyVis.VisElement({
  ...
  _makeSpans() {
    ...
    for (const idx in spans) {
      ...
      things.spans.push(new MyVis.VisElement({
  ...
  _makeChunks() {
    ...
    for (const idx in chunks_draft) {
      ...
      const chunk_ = new MyVis.VisElement({
        idx: idx,
        od: idx,
        chars: chunk,
        x: 0,
  ...
  _makeForcedNodesAndLinks() {
      ...
      for (const rela of ["ref", "clue"]) {
        for (const span_idx of unit?.[`${rela}_spans`]??[]) {
          if (this.things.spans.find(it=>it.idx==span_idx)) {
            span_unit_links.push(new MyVis.VisElement({
    ...
    for (const arc of (this?.data?.arcs??this?.data?.edges??this?.data?.tokens??[])) {
      ...
      unit_unit_links.push(new MyVis.VisElement({
  ...
  _drawChars() {
    ...
    const _eachFn = function (datum, iii, group) {
      ...
      chunk_text.selectAll("tspan.char-tspan").data(datum.chars).join(
        ...MyVis.joinFns("tspan", "char-tspan"),
      )
      .text(char=>char.text)
  ...
  _drawUnits() {
    ...
    const _eachFn = function (datum, iii, group) {
      ...
      datum._layers = {
        box: new MyVis.VisElement(),
        text: new MyVis.VisElement(),
        // joint: new MyVis.VisElement(),
      };
      ...
      MyVis.configSelection(box_rect, {
        attr: _attrGen(datum),
      });
      ...
      // const joint_points = MyVis._getJointPoints(box_layer_bbox);
  ...
  _makeAndDrawForces(realTimeResize=false) {
    ...
    //   MyVis.forceOrdering(this.forced_nodes_and_links.span_nodes)
    //     .axis("x").key(it=>it?.indices?.[0]).drift(-2)
    // );

    // 单元排序
    simulation.force("order-force-unit",
      MyVis.forceOrdering(this.forced_nodes_and_links.unit_nodes)
    ...
    simulation.on("tick", ()=>{
      ...
      const center_x = MyVis._getJointPoint(chunks_box, "center").x;
